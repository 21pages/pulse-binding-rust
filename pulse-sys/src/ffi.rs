...
static FUNCTIONS: once_cell::sync::OnceCell<Arc<PulseFunctions>> = once_cell::sync::OnceCell::new();
...
pub fn init() -> Result<(), Box<dyn std::error::Error>> {
    use crate::dynlib::get_pulse_library;

    if FUNCTIONS.get().is_some() {
        return Ok(());
    }

    let lib = get_pulse_library()
        .ok_or("Failed to get pulse library")?
        .lock()
        .map_err(|_| "Failed to lock pulse library")?;

    let lib = lib.as_ref().ok_or("Pulse library not loaded")?;
